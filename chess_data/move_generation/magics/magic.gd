# magic.gd - registered as autoload named Magic
extends RefCounted
...
func get_rook_attacks(square: int, blockers: int) -> int:
	var key: int = ((blockers & rook_mask[square]) * PrecomputedMagics.ROOK_MAGICS[square]) >> PrecomputedMagics.ROOK_SHIFTS[square]
	return rook_attacks[square][key]

func get_bishop_attacks(square: int, blockers: int) -> int:
	var key: int = ((blockers & bishop_mask[square]) * PrecomputedMagics.BISHOP_MAGICS[square]) >> PrecomputedMagics.BISHOP_SHIFTS[square]
...
func _create_table(square: int, rook: bool, magic: int, left_shift: int) -> Array:
	...
	for pattern in blocker_patterns:
		var index: int = (pattern * magic) >> left_shift
